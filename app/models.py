...
class Rig01(db.Model):
    ...
    Temp = db.Column(db.Float(), unique=False , nullable=True , default=None)
    API_Humidity = db.Column(db.Integer(), unique=False , nullable=True ,default=None)
    Pressure = db.Column(db.Float(), unique=False , nullable=True , default=None)
    WindSpeed = db.Column(db.Float(), unique=False , nullable=True , default=None)
    WindDeg = db.Column(db.Float(), unique=False , nullable=True , default=None)
    Weather = db.Column(db.String(16), unique=False , nullable=True , default=None)

    def __repr__(self):
        return f"Mesurment({self.dbTime_Stamp},{self.Time_Stamp}, {self.Temp1} ,{self.Temp2} ,{self.Tambiant}, {self.Humidity} , \
                            {self.Temp},{self.API_Humidity}, {self.Pressure} ,{self.WindSpeed} ,{self.WindDeg}, {self.Weather})"
